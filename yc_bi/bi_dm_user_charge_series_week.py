...
class table:
    def execute(self, argv):
        table = "bi_dm_user_charge_series_week"
        dir = "/tmp/"+table
        hdfs_1 = "/user/hive/warehouse/yc_ods_mid_bi.db/bi_dm_user_charge_series_week/"
        ...
        while start_time < end_time :
            ...
            print hdfs
            dt = dtoop()
            sql = """insert overwrite table yc_ods_mid_bi.bi_dm_user_charge_series_week partition(dt={dt_day})
            select count(user_id_charge_week),
            count(user_id_act_1week),
            count(user_id_act_2week),
            count(user_id_act_3week),
            count(user_id_act_4week),
            count(user_id_act_5week),
            count(user_id_act_6week),
            count(user_id_act_7week),
            count(user_id_act_8week),
            count(user_id_act_9week),
            count(user_id_act_10week),
            ...
            dt
            from yc_dm_mp.dm_user_charge_series_detail_info_week
