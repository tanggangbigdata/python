...
class table:
  def execute(self,argv):
    ...
    while start_time<end_time:
       ...
       sql1="""
        insert overwrite table test.dm_user_first_charge_test
...
from
(select *,row_number() over(partition by account_id order by create_time)num1 from yc_bit.account_history where reason=1 and type in(2,4) and dt<=20170401)t1
left join
(select *,row_number() over(partition by account_id order by create_time)num2 from yc_bit.account_history where reason=16 and type in(2,4) and dt<=20170401)t2
on t1.account_id=t2.account_id and t2.num2=1
left join
(select user_id,account_id,row_number() over(partition by user_id order by create_time desc)num3 from yc_ods.fu_user where dt<=20170401)t3
...
left join
(select *,row_number() over(partition by account_id order by create_time)num2 from yc_bit.account_history where reason=1 and type in(2,4) and dt={dt})t2
on t1.account_id=t2.account_id and t2.num2=1
left join
