...
class table:
  def execute(self,argv):
    ...
    while start_time<end_time:
       ...
       sql="""insert overwrite table yc_dm_mp.dm_user_charge_series_detail_info_week partition(dt={dt11})
       select t1.user_id,
       ...
       where dt>={dt11} and dt<={dt17} and status=7)t31
       on t1.user_id=t31.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt21} and dt<={dt27} and status=7)t32
       on t1.user_id=t32.user_id 
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt31} and dt<={dt37} and status=7)t33
       on t1.user_id=t33.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt41} and dt<={dt47} and status=7)t34
       on t1.user_id=t34.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt51} and dt<={dt57} and status=7)t35
       on t1.user_id=t35.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt61} and dt<={dt67} and status=7)t36
       on t1.user_id=t36.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt71} and dt<={dt77} and status=7)t37
       on t1.user_id=t37.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt81} and dt<={dt87} and status=7)t38
       on t1.user_id=t38.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt91} and dt<={dt97} and status=7)t314
       on t1.user_id=t314.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt101} and dt<={dt107} and status=7)t330
       on t1.user_id=t330.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt11} and dt<={dt17})t41
       on t1.user_id=t41.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt21} and dt<={dt27})t42
       on t1.user_id=t42.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt31} and dt<={dt37})t43
       on t1.user_id=t43.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt41} and dt<={dt47})t44
       on t1.user_id=t44.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt51} and dt<={dt57})t45
       on t1.user_id=t45.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt61} and dt<={dt67})t46
       on t1.user_id=t46.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt71} and dt<={dt77})t47
       on t1.user_id=t47.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt81} and dt<={dt87})t48
       on t1.user_id=t48.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt91} and dt<={dt97})t414
       on t1.user_id=t414.user_id
       left join (select distinct(user_id) as user_id from yc_dw_mp.dw_service_order_userid_unique_day 
       where dt>={dt101} and dt<={dt107})t430
       ...
       from_unixtime(t1.first_charge_time,'yyyyMMdd')>={dt11} and from_unixtime(first_charge_time,'yyyyMMdd')<={dt17}
